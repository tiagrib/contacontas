...
from data.database import CDDatabase, create_record_hash
from . import cls, manipulation as manip


class NumpandasDatabase(CDDatabase):
    def __init__(self):
        ...
        # account
        self.a = np.array([], dtype=np.dtype([("fk_acc", np.int32), ("name", "U32"), ("fk_bank", np.int32)]))

        # bank
        self.b = np.array([], dtype=np.dtype([("fk_bank", np.int32), ("name", "U32")]))
    ...
    def make_records_from_tagged(self, tag, bank_account, new_tag, invert_value=False):
        ...
        tagged_recs = self.get_tagged_records(tag)
        records = []
        value_mult = 1.0
        if invert_value:
            value_mult *= -1
        ...
        self.allocate(1, 1, len(records))
        for r in records:
            self._add_record_preallocated(*r, tag=new_tag, internal=True)
        self.finalize_loading_bank_account()
    ...
    def finalize_loading(self):
        self.m = self.m.reset_index(drop=True)
        self.preclassify()
    ...
    def preclassify(self):
        ...
        stack.add_classifier(
            cls.test_classifier(
                classes={
                    cls.t_and(cls.t_eq("bank", "PayPal"), cls.t_contains_words("desc", ["bank deposit to pp", "general credit card deposit", "top up", "general withdrawl - bank account"])): "paypal_cash_in",
                    cls.t_and(cls.t_eq("bank", "PayPal"), cls.t_contains_words("desc", ["general withdrawal - bank account", "general credit card withdrawal"])): "paypal_cash_out",
                    cls.t_and(cls.t_and(cls.t_eq("bank", "ActivoBank"), cls.t_eq("account", "Ordem")), cls.t_contains_words("desc", "pagamento cartao de credito")): "cc_cash_out",
                    cls.t_and(cls.t_and(cls.t_eq("bank", "ActivoBank"), cls.t_eq("account", "CC")), cls.t_contains_words("desc", "pagamento cartao de credito")): "cc_cash_in",
                    cls.t_and(cls.t_not(cls.t_eq("bank", "PayPal")), cls.t_and(cls.t_contains_words("desc", "paypal"), cls.t_lessthan("value", 0))): "to_paypal",
                    cls.t_and(cls.t_not(cls.t_eq("bank", "PayPal")), cls.t_and(cls.t_contains_words("desc", "paypal"), cls.t_greaterthan("value", 0))): "from_paypal",
                    cls.t_and(cls.t_and(cls.t_eq("bank", "ActivoBank"), cls.t_eq("account", "Ordem")), cls.t_contains_words("desc", "liq parcial dep prazo")): "from_poup",
                    cls.t_and(cls.t_and(cls.t_eq("bank", "ActivoBank"), cls.t_eq("account", "Ordem")), cls.t_contains_words("desc", "reforco dep prazo")): "to_poup",
                }
            )
        )
